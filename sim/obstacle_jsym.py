#!/usr/bin/env python3
"""Symmetric sketched maze: a hooked wall in the top + its exact 180-degree
rotation in the bottom. Right edge is sealed (no far-right escape) and the
optimal path is forced to double back. Saves to data/obstacle_report/00_map_jsym.png.
"""

import os
import sys

import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt
import numpy as np

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from agents.tabular_baseline import value_iteration_q
from envs.obstacle_gridworld import ObstacleGridWorld
from obstacle_lab import draw_policy_map, greedy_path, REPORT_DIR, GAMMA

G = 17
# upper hooked wall: attached to the LEFT edge, hook curling DOWN at its right end
UPPER = [(0.00, 0.30), (0.12, 0.285), (0.26, 0.305), (0.40, 0.29), (0.54, 0.30),
         (0.66, 0.315), (0.72, 0.37), (0.725, 0.45)]


def _bres(a, b):
    (r0, c0), (r1, c1) = a, b
    cells = set(); dr, dc = abs(r1 - r0), abs(c1 - c0)
    sr = 1 if r1 > r0 else -1; sc = 1 if c1 > c0 else -1
    err = dr - dc; r, c = r0, c0
    while True:
        cells.add((r, c))
        if (r, c) == (r1, c1):
            break
        e2 = 2 * err
        if e2 > -dc:
            err -= dc; r += sr
        if e2 < dr:
            err += dr; c += sc
    return cells


def symmetric_walls(G):
    pts = [(round(y * (G - 1)), round(x * (G - 1))) for (x, y) in UPPER]
    walls = set()
    for i in range(len(pts) - 1):
        walls |= _bres(pts[i], pts[i + 1])
    walls |= {(G - 1 - r, G - 1 - c) for (r, c) in walls}   # 180-degree rotation
    walls.discard((0, 0)); walls.discard((G - 1, G - 1))
    return {(r, c) for (r, c) in walls if 0 <= r < G and 0 <= c < G}


def main():
    os.makedirs(REPORT_DIR, exist_ok=True)
    walls = symmetric_walls(G)
    env = ObstacleGridWorld(G, G, walls=walls)
    q_star = value_iteration_q(env, gamma=GAMMA)
    opt_mask = q_star == q_star.max(axis=2, keepdims=True)
    vals = np.array([q_star[r, c].max() for (r, c) in env.all_states()])
    vmin, vmax = float(vals.min()), float(vals.max())
    path = greedy_path(lambda r, c: q_star[r, c], env)
    ups = sum(1 for a, b in zip(path, path[1:]) if b[0] < a[0] or b[1] < a[1])
    sym = all((G - 1 - r, G - 1 - c) in walls for (r, c) in walls)

    fig, ax = plt.subplots(figsize=(8.5, 8.5))
    draw_policy_map(ax, lambda r, c: q_star[r, c], env, opt_mask,
                    "Symmetric maze (optimal policy)", vmin, vmax, show_path=True)
    fig.suptitle(f"Symmetric maze ({G}x{G}, 180-deg) — walls=black, start=cyan, "
                 f"goal=gold star, white=optimal path", fontsize=11, fontweight="bold")
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    out = os.path.join(REPORT_DIR, "00_map_jsym.png")
    plt.savefig(out, dpi=120, bbox_inches="tight"); plt.close(fig)
    print(f"[OK] {out}: {len(env.walls)} walls, path len {len(path)-1}, "
          f"backward moves {ups}, symmetric {sym}, reaches goal {path[-1]==env.goal}")


if __name__ == "__main__":
    main()
