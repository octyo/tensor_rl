...
class GraphicsUtilGym:
    ...
    def begin_graphics(self, width=640, height=480, color=formatColor(0, 0, 0), title="02465 environment", local_xmin_xmax_ymin_ymax=None, verbose=False,
        ...
        if os.path.isdir(icon := os.path.dirname(__file__) + "/../utils/graphics/dtu_icon.png"):
            pygame.display.set_icon(pygame.image.load(icon))
        screen_width = width
        screen_height = height
        pygame.init()
        # pygame.display.init() # Is this required?

        self.clock = pygame.time.Clock() # Is this required?
    ...
    def scale(self):
        dx = self._local_xmin_xmax_ymin_ymax[1] - self._local_xmin_xmax_ymin_ymax[0]
        return self.screen_width / dx
    ...
    def close(self):
        ...
        from rl_course.utils import ptext
        ptext._font_cache = {}
        self.isopen = False
    ...
    def blit(self, render_mode=None):
        ...
        if render_mode == "human" :
            ...
            pygame.display.flip()
            self.clock.tick(120)  # wait until next frame (at 60 FPS)
            # pygame.display.update()
    ...
    def polygon(self, name, coords, outlineColor=None, fillColor=None, filled=True, smoothed=1, behind=0, width=1.0, closed=False):
        ...
        if not filled:
            ...
            pygame.draw.polygon(self.surf, h2rgb255(fillColor), coords)
    ...
    def circle(self, name, pos, r, outlineColor=None, fillColor=None, endpoints=None, style='pieslice', width=2):
        ...
        if endpoints is not None and len(endpoints) > 0:
            ...
            pygame.draw.circle(self.surf, h2rgb255(fillColor), (x, y), math.ceil(r), width=0)
    ...
    def text(self, name, pos, color, contents, font='Helvetica', size=12, style='normal', anchor="w", fontsize=24,
        ...
        from rl_course.utils.ptext import draw
        if anchor == 'w':
...
class Object(pygame.sprite.Sprite):
    ...
    def flip(self,  flip_x=True, flip_y=False):
        self.surf = pygame.transform.flip(self.og_surf, flip_x=flip_x, flip_y=flip_y)#(self.og_surf, self.angle)
