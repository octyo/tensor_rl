...
from rl_course.utils.common import load_time_series, log_time_series
from rl_course.utils.rl_course_plot import existing_runs
import shutil
...
class Agent: 
    ...
    :Example:
        ...
        .. runblock:: pycon
            ...
            >>> from rl_course import Agent                                              # You can import directly from top-level package
            >>> import numpy as np
            >>> np.random.seed(42)                                                  # Fix the seed for reproduciability
            >>> from rl_course.ex01.inventory_environment import InventoryEnvironment
        ...
        * ``info_sp`` is the information-dictionary returned by the environment as it transitioned to ``sp``.
        ...
        :Example:      
            ...
            .. runblock:: pycon
                ...
                >>> from rl_course.ex01.inventory_environment import InventoryEnvironment    # import environment
                >>> from rl_course import Agent
                >>> env = InventoryEnvironment()                                        # Create an instance of the environment
                >>> agent = Agent(env)                                                  # Create an instance of the agent.
...
def train(env,
    ...
    from rl_course import cache_write
    from rl_course import cache_read
    saveload_model = False
    # temporal_policy = None
    save_stats = True
    if agent is None:
        print("[train] No agent was specified. Using rl_course.Agent(env) (this agent selects actions at random)")
        agent = Agent(env)
    ...
    with tqdm(total=num_episodes, disable=not verbose, file=sys.stdout, mininterval=int(num_episodes/100) if num_episodes>100 else None) as tq:
        for i_episode in range(num_episodes): 
            ...
            if return_trajectory:
                try:
                    from rl_course.ex04.control_environment import ControlEnvironment
